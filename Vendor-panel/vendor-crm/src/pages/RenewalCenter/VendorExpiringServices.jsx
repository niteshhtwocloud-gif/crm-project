...
export default function VendorExpiringServices() {
  ...
  const expiringList = useMemo(() => {
    ...
    users.forEach(u => {
      ...
      if ((dLeft !== null && dLeft > 0 && dLeft <= 7) || u.status === 'Expiring') {
        combined.push({
          id,
    ...
    services.forEach(s => {
      ...
      if ((dLeft !== null && dLeft > 0 && dLeft <= 7) || s.status === 'Expiring') {
        combined.push({
          id,
  ...
  const handleRenewRequest = async (item) => {
    if (!window.confirm(`Send renewal request for "${item.customer} - ${item.product}"?`)) return;
        ...
        <div className="table-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <p style={{ fontSize: '13px', color: '#64748b', margin: '4px 0 0 0' }}>
              Services and customer licenses requiring renewal within the next 7 days — {expiringList.length} total.
            </p>
          </div>
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="strong">
                    <span style={{ background: 'rgba(245, 158, 11, 0.15)', color: '#D97706', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: 600 }}>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} className="empty-row" style={{ textAlign: 'center', padding: '40px' }}>
                    No services expiring in the next 7 days.
                  </td>
                </tr>
              )}
            </tbody>
