...
export function isCustomerExpired(expiryDateVal) {
  if (!expiryDateVal) return false;
  const isoExpiry = toISODate(expiryDateVal);
  if (!isoExpiry) return false;
  const parts = isoExpiry.split('-');
...
export function getEffectiveStatus(customer) {
  const expiryDate = customer?.expiryDate || customer?.expiry;
